# -*- coding: utf-8 -*-
"""
Created on Fri Jul  5 17:22:04 2024
...
for OB in ['Southern Ocean', 'Arctic Ocean']:    
    ...
    sals = np.zeros((len(data.depth), nmonths))*np.nan
    area_valid = A_3d * OF
    ...
    for year in range(startyear, endyear+1):
        for month in range(1, 13):
            ...
            data = xr.open_dataset(f'{datadir}/{filename}')
            temp = data.salinity.to_numpy().squeeze()
            ...
            sals[:, timeidx] = onemonth
    
    mean_sals= np.nanmean(sals, axis = 1)
    mean_sals_2d = np.zeros(sals.shape)
    for i in range(len(mean_sals)):
        mean_sals_2d[i, :] = mean_sals[i]
        
    sals_diff = (sals - mean_sals_2d)
    
    maxval = max(np.abs(np.nanmin(sals_diff)), np.nanmax(sals_diff))
    ...
    plot = ax.contourf(x_time, data.depth, sals_diff, cmap = 'seismic', norm = norm)
    ax.set_yscale('log')
    plt.colorbar(plot, label = r"$S'$", location = 'bottom')
    ax.set_ylabel('Depth')
    ax.set_xlabel('Time')
    ...
    fig.savefig(f'EN4 Plots/TimeVsDepth/Salinity/{OB}_TimeVsDepth_{SO_cutoff_lat}.pdf', bbox_inches = 'tight')
