# -*- coding: utf-8 -*-
"""
Created on Sat Jul 13 16:18:14 2024
...
volume_valid = data.salinity.values[0,:, :, :]/data.salinity.values[0,:, :, :]
...
for method in ['BAR', 'PYC']:
    ...
    for month in range(1, 13):
        ...
        APE_700 = np.sum(APE_all*depth_fracs/Am, axis = 0)/zsum
    ...
    for month_i in range(anomalies.shape[0]):
        ...
        negplot = ax.imshow(np.flip(logneg, axis = 0), 
                         extent = extent, cmap = 'Blues', vmin = vmin, vmax = vmax)#, norm=colors.CenteredNorm())#, norm=colors.SymLogNorm(linthresh = 1, vmin=-4e15, vmax=4e15, base=1e14))
    ...
    fig.suptitle('Vertically Averaged APE $[Jm^{-3}]$' + f' Anomalies: WOCE {method}', y = 0.95, fontsize = fs)
